...
object MainDestinations {
    ...
    const val TOUR_DETAIL_ROUTE = "tour"
    const val ATTRACTION_DETAIL_ROUTE = "attraction"
    const val SNACK_ID_KEY = "snackId"
    const val TOUR_ID_KEY = "tourId"
    const val ATTRACTION_ID_KEY = "attractionId"
...
class JetsnackNavController(val navController: NavHostController) {
    ...
    fun navigateToTourDetail(tourId: Long, from: NavBackStackEntry) {
        // In order to discard duplicated navigation events, we check the Lifecycle
//        if (from.lifecycleIsResumed()) {
            navController.navigate("${MainDestinations.TOUR_DETAIL_ROUTE}/$tourId")
//        }
    }

    fun navigateToAttraction(attractionId: Long, from: NavBackStackEntry) {
        // In order to discard duplicated navigation events, we check the Lifecycle
        if (from.lifecycleIsResumed()) {
            navController.navigate("${MainDestinations.ATTRACTION_DETAIL_ROUTE}/$attractionId")
        }
    }
